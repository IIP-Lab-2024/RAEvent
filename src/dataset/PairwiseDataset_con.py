...
class PairwiseDataset1(Dataset):
    def __init__(self, mode, encoding="utf8", *args, **params):
        self.mode = mode

        self.query_path = "/hhd2/fan/SCR/output_data-supervised/query/query_5fold_att"
        self.cand_path = "/hhd2/fan/SCR/output_data-supervised/candidates_att"
        self.labels = json.load(open("/hhd2/fan/SCR/SCR-Experiment/input_data/label/golden_labels.json", 'r'))
        self.data = []
        ...
        for i in range(5): # 0,1,2,3,4
            if mode == 'train':
                if test_file == str(i): # test_file=0  改变这个来实现五折交叉验证
                    continue
